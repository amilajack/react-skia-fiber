...
export default function App() {
  ...
  useFrame(() => {
    ...
    rects.forEach((rect, i) => {
      rect.ref.current!.x =
        (rect.simplex.noise2D(i + now, 0) + 1) * window.innerWidth;
      rect.ref.current!.y =
        (rect.simplex.noise2D(i + 1 + now, 0) + 1) * window.innerHeight;
      rect.ref.current!.paint.setColor(
        canvasKit.Color4f(
          (rect.simplex.noise2D(i + 0.1 + now, 0) + 1) / 2,
          (rect.simplex.noise2D(i + 0.2 + now, 0) + 1) / 2,
          (rect.simplex.noise2D(i + 0.3 + now, 0) + 1) / 2,
          1.0
        )
      );
